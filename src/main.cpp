...
void onSaveBody(AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total) {
  ...
  savedCodes.end();
  g_savedCodesCache.push_back({String(buf), String(doc["name"] | ""), String(doc["protocol"] | "UNKNOWN"), String((const char*)(doc["value"] | "0")), doc["bits"] | 32});
...
static bool saveImportedCodes(JsonArray in, JsonDocument &outDoc) {
  ...
  for (size_t i = 0; i < in.size(); i++) {
    ...
    String key = String(n);
    savedCodes.putString(key.c_str(), buf);
    g_savedCodesCache.push_back({String(buf), String(entry["name"] | ""), String(entry["protocol"] | "UNKNOWN"), String((const char*)(entry["value"] | "0")), entry["bits"] | 32});
    n++;
...
void handleSavedRename(AsyncWebServerRequest *request) {
  ...
  savedCodes.putString(key.c_str(), buf);
  savedCodes.end();
  g_savedCodesCache[index] = {String(buf), String(entry["name"] | ""), String(entry["protocol"] | "UNKNOWN"), String((const char*)(entry["value"] | "0")), entry["bits"] | 32};
...
void handleDump(AsyncWebServerRequest *request) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    snprintf(fragBuf, sizeof(fragBuf), "// %d %s %s 0x%s %db\n", i, name, protocol, valueHex, bits);
    out += fragBuf;

    if (g_savedCodesCache[i].protocol.equalsIgnoreCase("NEC")) {
      snprintf(fragBuf, sizeof(fragBuf), "irsend.sendNEC(0x%su, %d);  // %s\n", valueHex, bits, name);
    } else {
      snprintf(fragBuf, sizeof(fragBuf), "// irsend.send... (unsupported protocol); value=0x%s %s\n", valueHex, name);
    }
    out += fragBuf;
  }
