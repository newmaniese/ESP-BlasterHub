...
Pure helper functions (`replayUrlFor`, `saveUrlFor`) are extracted into `include/ir_utils.h` / `src/ir_utils.cpp` and tested with Unity on the ESP32-C3. The `esp32c3-test` env builds only the helpers (no WiFi, IR, or NVS).
